...
contract Academy { 
    ...
    // Enums are enumerated values, they allow for new types, similar to factors in R, they are convertible to integers
    // but all coercion must be explicit
    
    // Create a human readible type "Status" with constant members: Open, Closed, Unknown
    enum State {Open, Closed, Unknown}  // no semi colon needed for enums
    
    State public academyState = State.Open; // the school is open, closed, or unknown
    
    // makes conditional ifelse easier, see below, changeInstructor only works if academy is open 
